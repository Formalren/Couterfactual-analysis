...
BASE_DEFAULT_CONFIG = dict(
    #random.randint(),这个函数有两个参数，分别是范围的开始和结束，返回的是这两个参数之间的随机整数（包括这两个参数）。
    #round(random.uniform(a, b), n) 函数用于生成一个在[a, b]范围内的随机实数，‌并将其四舍五入到指定的小数位数n。‌
    # ===== agent =====
    ...
    vehicle_config=dict(
        ...
        #lidar=dict(
        #    num_lasers=random.randint(150, 350), distance=random.randint(50, 200), num_others=0, gaussian_noise=random.randint(0, 1), dropout_prob=random.uniform(0,0.2), add_others_navi=False #gaussian_noise=0.0
        #),
        lidar=dict(
            num_lasers=240, distance=50, num_others=0, gaussian_noise=0.0, dropout_prob=0.0, add_others_navi=False #gaussian_noise=0.0
        ),
        #side_detector=dict(num_lasers=0, distance=random.randint(50, 200), gaussian_noise=random.uniform(0, 0.2), dropout_prob=random.uniform(0, 0.2)),
        #lane_line_detector=dict(num_lasers=0, distance=random.randint(50, 200), gaussian_noise=random.uniform(0, 0.2), dropout_prob=random.uniform(0, 0.2)),
...
class BaseEnv(gym.Env):
  ...
  #用于定义类方法，类方法在调用时，第一个参数是类本身，通常将其命名为 cls。使用 @classmethod 装饰器可以让 Python 解释器知道这是一个类方法，从而正确地处理它。
    @classmethod
    ...
    def step(self, actions: Union[Union[np.ndarray, list], Dict[AnyStr, Union[list, np.ndarray]], int]):
        ...
        return self._get_step_return(actions, engine_info=engine_info)  # collect observation, reward, termination，收集观察、奖励、终止
    ...
    def reset(self, seed: Union[None, int] = None):
        ...
        :return: None
        重置环境，可以通过提供 episode_data 来恢复和重播场景
        重置环境或从情节数据中加载情节以恢复
        :param seed: 设置环境的种子。它实际上是您打算选择的场景索引
        :return: None
        """
    ...
    def _wrap_info_as_single_agent(self, data):
        """
        Wrap to single agent info 包裹至单个代理信息
        """
    ...
    def _get_step_return(self, actions, engine_info):
        # update obs, dones, rewards, costs, calculate done at first ! 更新观察、完成、奖励、成本，首先计算完成！
        obses = {} #观察
        done_infos = {} #完成
        cost_infos = {} #花费
        reward_infos = {} #奖励
        ...
        # 仅适用于极端情况。如果环境步骤超过 5 倍范围，则强制终止所有代理。
        ...
        if not self.is_multi_agent:
            ...
            return obses, rewards, terminateds, truncateds, step_infos #观察、奖励、终止、截断、步骤
