...
from Test2_simulation_TorchExpert import run_simulation
from metadrive.component.sensors.rgb_camera import RGBCamera
from metadrive.policy.expert_policy import ExpertPolicy
from metadrive.policy.torch_policy import TorchPolicy
import pandas as pd
import random
import argparse
...
# 检查前100条数据并更新参数
for index, row in df.iloc[1050:1064].iterrows():
            ...
            for _ in range(10):  # 每个参数进行10次模拟
                ...
                if current_params[0] == 'map':  # If the first parameter is 'map'
                    ...
                    if param == 'traffic_density':
                        while True:
                            random_value = random.choice([0.2, 0.3, 0.4])
                    ...
                    if param == 'num_scenarios':
                        while True:
                            random_value = random.choice([500, 750, 1000])
                    ...
                    if param == 'lidar_dropout_prob':
                        while True:
                            random_value = random.choice([0.2, 0.4, 0.6])
                    ...
                    if param == 'side_detector_distance':
                        while True:
                            random_value = random.choice([30, 40, 50])
                    ...
                    if param == 'side_detector_gaussian_noise':
                        while True:
                            random_value = random.choice([0.1, 0.3, 0.5])
