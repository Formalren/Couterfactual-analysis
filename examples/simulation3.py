...
def run_simulation(config, args):
    ...
    config_file_exists = os.path.isfile('output_verify_filter_new7.csv')
    ...
    try:
        ...
        # print(f"Final info_to_write444: {info_to_write}")
        # crash_vehicle = info.get("crash_vehicle", None)
        # print(f"Final crash_vehicle: {crash_vehicle}")

        # # 如果 crash_vehicle 是 0，记录该信息
        # if crash_vehicle == 0:
        #     crash_info = filter_info(info)  # 仅当 crash_vehicle 为 0 时才保存信息
        #     print("Crash occurred, crash_info:", crash_info)
        # 写入 config 和 info 数据到文件
        with open('output_verify_filter_new7.csv', 'a', newline='', encoding='utf-8') as f:
